...
function eHitTest(wx, wy) {
  ...
  // Coins
  for (let i = ld.coins.length - 1; i >= 0; i--) {
    const c = ld.coins[i];
    if (Math.abs(wx - c.x) < 10 && Math.abs(wy - c.y) < 10)
      return { col: 'coins', idx: i };
  }
  return null;
}
...
function eGetXY(sel) {
  ...
  if (sel.col === 'coins')     return { x: ld.coins[sel.idx].x,     y: ld.coins[sel.idx].y };
...
function eSetXY(sel, x, y) {
  ...
  else if (sel.col === 'coins')    { ld.coins[sel.idx].x = x;   ld.coins[sel.idx].y = y; }
...
function eDeleteSelected() {
  ...
  else if (col === 'coins')    ld.coins.splice(idx, 1);
